...
class LoI(nn.Module):
    def __init__(self, order, n_pts=32):
    ...
    def forward(self, feature, loi_pred):
        ...
        * math:`P`: number of the sample points.
        ...
        loi_feature = (feature[:, py0l, px0l] * (py1 - py) * (px1 - px) + \
            ...
            feature[:, py1l, px1l] * (py - py0) * (px - px0)).reshape(ch, -1, self.n_pts).permute(1, 0, 2)

        return loi_feature


class LoIHead(nn.Module):
    def __init__(self, order, num_feats=256, n_pts=32):
